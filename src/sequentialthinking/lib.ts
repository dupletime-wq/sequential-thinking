...
/**
 * Registers the `sequentialthinking` tool on an MCP server. Shared between
 * the stdio server (index.ts) and the Cloudflare Worker (worker.ts) so both
 * transports expose the exact same tool contract.
 */
export function registerSequentialThinkingTool(server: McpServer, thinkingServer: SequentialThinkingServer): void {
  server.registerTool(
    "sequentialthinking",
    {
